...
class GlobalExceptionHandlerTest {
    ...
    @Test void validationContainsCleanFieldErrors() throws Exception {
        mvc.perform(post("/errors/validation").contentType(MediaType.APPLICATION_JSON)
                        .header("Accept-Language", "tr-TR").content("{}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errorCode").value("VALIDATION_ERROR"))
                .andExpect(jsonPath("$.message").value("Gönderilen bilgiler geçerli değil."))
                .andExpect(jsonPath("$.fieldErrors.name").exists());
    }

    @Test void optionalEnglishProductNameStaysOptionalButRealShortValueIs400() throws Exception {
        String validBase = "{\"name\":\"Ürün\",\"code\":\"P-1\",\"lotNumber\":\"LOT-1\",";
        mvc.perform(post("/errors/product").contentType(MediaType.APPLICATION_JSON)
                        .content(validBase + "\"nameEn\":\"   \"}"))
                .andExpect(status().isOk());

        mvc.perform(post("/errors/product").contentType(MediaType.APPLICATION_JSON)
                        .header("Accept-Language", "tr-TR")
                        .content(validBase + "\"nameEn\":\"a\"}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errorCode").value("VALIDATION_ERROR"))
    ...
    static class ThrowingController {
        ...
        @PostMapping("/product") void product(@Valid @RequestBody ProductRequest input) {}
